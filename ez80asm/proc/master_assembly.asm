...
;
OC_equs:			EQU     STAVAR+15*4     ; CODE ORIGIN (O%)
PC_equs:			EQU     STAVAR+16*4     ; PROGRAM COUNTER (P%)
...
ASC_TO_NUMBER:		PUSH	BC			; Preserve BC
			...
			CALL	SKIPSP_agon_misc			; Skip whitespace
...
ASC_TO_NUMBER1:		LD	A, (HL)			; Fetch the character
			CALL    UPPRC_agon_misc			; Convert to uppercase  
...
ASC_TO_NUMBER3:		LD	A, (HL)
			...
			INC	HL
			JR	ASC_TO_NUMBER3
ASC_TO_NUMBER4:		POP	BC 			; Fall through to SKIPSP_agon_misc here

; Skip a space
; HL: Pointer in string buffer
; 
SKIPSP_agon_misc:			LD      A, (HL)
			CP      ' '
			RET     NZ
			INC     HL
			JR      SKIPSP_agon_misc
...
CSTR_ENDSWITH:		LD	A, (HL)			; Get the source string byte
			CALL	UPPRC_agon_misc			; Convert to upper case
			LD	C, A
...
LNZERO:			LD	DE,BUFFER	
                	...
                	JR	Z,ATEND_agon_os	
                	LDDR				; MAKE SPACE	
ATEND_agon_os:          	POP	BC			; LINE LENGTH	
                	POP	DE			; LINE NUMBER	
                	INC	HL	
...
SOUND__agon_os:			RET ; TEMPORARY

; OSWORD
;
OSWORD:			CP	0x07			; SOUND
			JR	Z, OSWORD_07
			CP	0x08			; ENVELOPE
			JR	Z, OSWORD_08
			CP	0x09			; POINT
...
; - H: Second parameter
;
OSBYTE:			CP	0x0B			; Keyboard auto-repeat delay
			JR	Z, OSBYTE_0B
...
SKIPSP_agon_os:			LD      A,(HL)			
        		CP      ' '
        		RET     NZ
        		INC     HL
        		JR      SKIPSP_agon_os	

UPPRC_agon_os:  		AND     0x7F
			CP      '`'
			RET     C
			AND     0x5F			; CONVERT TO UPPER CASE
			RET	
...
;   - 6,7: Duration -1 to 254 (duration in 20ths of a second, -1 = play forever)
;
SOUND__agon_sound:			LD	A, (HL)			; Channel
			LD	(VDU_BUFFER+0), A 
			XOR	A			; Waveform
			LD	(VDU_BUFFER+1), A
			INC	HL
...
OC_data:             EQU	STAVAR+15*4	;CODE ORIGIN (O%)	
PC_data:             EQU	STAVAR+16*4	;PROGRAM COUNTER (P%)	
...
FUNTBL:         DW	DECODE		;Line number	
                ...
                DW	PAGEV_eval		;PAGE	
                DW	TIMEV_eval		;TIME	
                DW	LOMEMV_eval		;LOMEM	
                DW	HIMEMV_eval		;HIMEM	
                DW	ABSev		;ABS	
                DW	ACS_eval		;ACS	
                DW	ADVAL		;ADVAL	
                DW	ASC		;ASC	
...
TCMD_eval:           EQU	FUNTBL_END-FUNTBL/2+FUNTOK
;
; CR             EQU	0x0D	
LF_eval:             EQU	0x0A	
AND_:           EQU	0x80	
...
EXPR3S:         EX	AF,AF'	
                ...
                LD	E,A		;DESTINATION	
                LD	A,19	
                JR	C,ERROR2_eval	;"String too long"	
                PUSH	DE	
                DEC	E	
                DEC	L	
                LDDR			;COPY	
...
;
BADHEX:         LD	A,28	
ERROR2_eval:         JP	ERROR_		;"Bad HEX_eval or binary"	
;
NEGATE_eval:         EXX	
                LD	A,H	
                CPL	
...
ITEM:           CALL	CHECK	
                ...
                JR	C,ITEM0	
                CP	TCMD_eval	
                JP	C,DISPAT_eval	;FUNCTIONS	
                JP	EXTRAS_eval		;DIM, END, MODE, REPORT$, WIDTH	
;
ITEM0:          CP	':'	
                JR	NC,ITEM2	;VARIABLES	
                CP	'0'	
                JR	NC,CON_eval		;NUMERIC CONSTANT	
                CP	'('	
                ...
                CP	'.'	
                JR	Z,CON_eval		;NUMERIC CONSTANT	
                CP	'&'	
                JR	Z,HEX_eval		;HEX CONSTANT	
                CP	'%'	
                JR	Z,BIN		;BINARY CONSTANT	
                CP	'"'	
                JR	Z,CONS		;STRING CONSTANT	
                CP	TTINT	
                JP	Z,TINT_eval		;TINT FUNCTION	
ITEM2:          CP	TMOD	
                JP	Z,MODFUN	;MOD	
                CP	'^'	
...
LOAD1:          LD	HL,0	
                ...
                RET	
;
NOSUCH_eval:         JP	C,SYNTAX	
                LD	A,(LISTON)	
                BIT	5,A	
                LD	A,26	
                JR	NZ,ERROR0_eval	;"No such variable"	
NOS1:           INC	IY	
                CALL	RANGE	
                JR	NC,NOS1	
                LD	IX,PC	
...
CONS2:          LD	A,(IY)	
                ...
                RET	
;
ARRAY_eval:          LD	A,14		;'Bad use of array'	
                JP	ERROR_	
;
...
GETAR1:         LD	A,(HL)	
                ...
                AND	0FEH	
                OR	H	
                JR	Z,ARRAY_eval		;Bad use of array	
                RET	
;
GETARB:         CALL	NXT	
                CP	'('	
...
LOADS2:         LD	A,(HL)	
                ...
                INC	HL	
                CP	CR	
REPDUN:         LD	A,0x80		;STRING MARKER	
                RET	Z	
                INC	E	
                JR	NZ,LOADS2	
...
ASC0:           XOR	A	
                CP	E	
                JP	Z,TRUE_eval		;NULL STRING	
ASC1:           LD	HL,(ACCS)	
                JR	COUNT0	
LEN:            CALL	ITEMS	
                EX	DE,HL	
                JR	COUNT0	
LOMEMV_eval:         LD	HL,(LOMEM)	
                JR	COUNT1	
HIMEMV_eval:         LD	HL,(HIMEM)	
                JR	COUNT1	
PAGEV_eval:          LD	HL,(PAGE_)	
                JR	COUNT1	
TOPV:           LD	A,(IY)	
                INC	IY		;SKIP "P"	
                CP	'P'	
...
RND5:           CALL	ITEMI	
                ...
                EX	DE,HL	
                CALL	LOAD5	
                CALL	NZ,RND1		;NEXT IF NON-ZERO_eval	
                EXX			;SCRAMBLE (CARE!)	
                LD	C,0x7F	
RND6:           BIT	7,H		;FLOAT	
                JR	NZ,RND7	
...
SUMLEN:         INC	IY		;Skip LEN	
                CALL	GETARB	
                BIT	7,B	
                JP	Z,MISMAT_eval	;Type mismatch	
                CALL	ARRLEN	
                PUSH	HL	
...
;Result is integer numeric.
;
INSTR:          CALL	EXPRS		;STRING TO SEARCH_eval	
                CALL	COMMA	
                CALL	PUSHS		;SAVE STRING ON STACK	
...
ITEMA1:         PUSH	DE		;Type and operator	
                ...
                RES	6,A	
                CP	D	
                JP	NZ,MISMAT_eval	;'Type mismatch'	
...
ARRDOT:         INC	IY		;Bump past dot	
                LD	A,D		;Type	
                OR	A	
                JP	M,MISMAT_eval	;'Type mismatch'	
                EX	DE,HL	
                POP	HL	
;
; A = type
...
OUTER:          PUSH	BC		;1	
                ...
                LD	E,C	
                PUSH	AF	
                CALL	ZERO_eval		;Zero accumulator	
                POP	AF	
INNER:          PUSH	DE		;6	
...
TTO_exec:            EQU	0B8H	
TCMD_exec:           EQU	0C0H	
TWHILE_exec:         EQU	0C7H	
TWHEN_exec:          EQU	0C9H	
TOF_exec:            EQU	0CAH	
TENDCASE_exec:       EQU	0CBH	
TOTHERWISE_exec:     EQU	0CCH	
TENDIF_exec:         EQU	0CDH	
TENDWHILE_exec:      EQU	0CEH	
TCALL_exec:          EQU	0D6H	
TDATA_exec:          EQU	0DCH	
TDEF_exec:           EQU	0DDH	
TFOR_exec:           EQU	0E3H	
TGOSUB_exec:         EQU	0E4H	
TGOTO_exec:          EQU	0E5H	
TLOCAL_exec:         EQU	0EAH	
TNEXT_exec:          EQU	0EDH	
TON_exec:            EQU	0EEH	
TPROC_exec:          EQU	0F2H	
TREM_exec:           EQU	0F4H	
TREPEAT_exec:        EQU	0F5H	
TRETURN:        EQU	0F8H	
TSTOP:          EQU	0FAH	
TUNTIL_exec:         EQU	0FDH	
TEXIT_exec:          EQU	0x10	
;
CMDTAB_exec:         DW	LEFTSL	
                DW	MIDSL	
                DW	RITESL	
                DW	SYNTAX	;STR$	
                DW	SYNTAX	;STRING$	
...
TLAST:          EQU	CMDTAB_END-CMDTAB_exec/2+TCMD_exec-128
;
RUN:            CALL	TERMQ	
                JR	Z,RUN0	
CHAIN:          CALL	EXPRS	
...
CHAIN0:         LD	SP,(HIMEM)	
                CALL	LOAD0	
RUN0:           LD	SP,(HIMEM)	;PREPARE FOR_exec RUN	
                LD	IX,RANDOM	
...
CLI:            CALL	EXPRS	
                ...
                JR	XEQ	
;
EXTRAS_exec:         CP	TELSE_exec-TCMD_exec	
                JR	Z,REM		;ELSE	
                CP	TERROR_exec-TCMD_exec	
                JR	Z,THROW		;ERROR	
                CP	TLINE_exec-TCMD_exec	
                JP	Z,LINE		;LINE	
                CP	TOFF-TCMD_exec	
                JP	Z,CSROFF	;OFF	
                JP	SYNTAX	
...
VAR_:           CALL	GETVAR	
                RET	Z	
                JP	NC,PUTVAR	
NOSUCH_exec:         LD	A,26		;'No such variable'	
                DB	0x21	
...
BADUSE:         LD	A,14		;'Bad use of array'	
                DB	0x21	
MISMAT_exec:         LD	A,6		;'Type mismatch'	
...
ON4:            LD	A,(IY)	
                ...
                LD	A,40		;'ON range'	
                DB	0x21	
FORVAR:         LD	A,34		;'FOR_exec variable'	
ERROR2_exec:         JP	ERROR_	
;
ONPROC:         LD	A,TON_exec	
                JP	PROC	
;
;GOTO line
;
...
UNTIL:          POP	BC	
                ...
                JR	NZ,UNTIL	
                LD	A,43	
                JR	ERROR2_exec		;"Not in a REPEAT loop"	
;
UNTIL1:         CALL	EXPRI	
                CALL	TEST	
                POP	BC	
                POP	DE	
                JR	NZ,XEQGO2_exec		;TRUE	
                PUSH	DE	
                PUSH	BC	
                PUSH	DE	
                POP	IY	
...
;
FOR_exec:            CALL	ASSIGN	
                JR	NZ,FORVAR	;"FOR_exec variable"	
                PUSH	AF		;SAVE TYPE	
                LD	A,(IY)	
                CP	TTO_exec	
                LD	A,36	
                JR	NZ,ERROR2_exec	;"No TO"	
                INC	IY	
                PUSH	IX	
                CALL	EXPRN		;LIMIT	
                POP	IX	
...
NEXT:           POP	BC		;MARKER	
                ...
                JR	NZ,NEXT	
                LD	A,32	
                JR	ERROR3_exec		;"Not in a FOR_exec loop"	
;
NEXT2:          CALL	TERMQ	
                POP	HL	
                PUSH	HL	
...
NEXT1:          LD	HL,18	
                ...
                JR	Z,NEXT0	
                LD	A,33	
ERROR3_exec:         JP	ERROR_		;"Can't match FOR_exec"	
;
;FNname
...
PROC1:          CALL	CHECK	
                ...
                JR	Z,PROC4	
                LD	A,30	
                JR	C,ERROR3_exec	;"Bad call"	
                PUSH	BC	
                LD	HL,(PAGE_)	
PROC2:          LD	A,TDEF_exec	
                CALL	SEARCH_exec		;LOOK FOR_exec "DEF"	
                JR	C,PROC3	
                PUSH	HL	
                POP	IY	
                INC	IY		;SKIP DEF	
...
PROC3:          POP	IY		;RESTORE TEXT POINTER	
                CALL	GETDEF	
                LD	A,29	
                JR	NZ,ERROR3_exec	;"No such FN/PROC"	
PROC4:          LD	E,(HL)	
                INC	HL	
                LD	D,(HL)		;GET ADDRESS	
                LD	HL,2	
...
IFNEXT:         CP	'"'	
                ...
                CP	TTHEN_exec	
                JR	NZ,IFNOT	;TRY FOR_exec END AGAIN	
IFTHEN:         LD	A,(IY)	
                CP	CR	
                JR	NZ,IFELSE	
                LD	BC,TELSE_exec	
                LD	DE,TENDIF_exec*256+TTHEN_exec	
                INC	IY	
                CALL	NSCAN	
                JP	Z,XEQ1	
NENDIF:         LD	A,49	
ERROR4_exec:         JP	ERROR_		;"Missing ENDIF"	
;
; ELSE (multi-line)
;
MELSE:          LD	BC,-3	
...
WHEN:           LD	BC,-3	
                ADD	IY,BC	
                LD	BC,TENDCASE_exec	
                LD	DE,TENDCASE_exec*256+TOF_exec	
                CALL	NSCAN	
                JR	Z,XEQGO7	
                LD	A,47	
                JR	ERROR4_exec		;"Missing ENDCASE"	
;
; CASE
;
...
CASE6:          LD	A,(IY)	
                ...
                CP	TOF_exec	
                LD	A,37	
                JR	NZ,ERROR4_exec	;"Missing OF"	
                LD	A,(IY)	
                INC	IY		;Address line-length byte	
                CP	CR	
                LD	A,48	
                JR	NZ,ERROR4_exec	;"OF not last"	
CASE1:          XOR	A		;Level	
CASE0:          EXX	
                ...
                PUSH	BC	
                LD	L,A		;Level	
                LD	BC,TOTHERWISE_exec*256+TWHEN_exec	
                LD	DE,TENDCASE_exec*256+TOF_exec	
                CALL	NSCAN1	
...
CASE2:          LD	A,(IY)	
                ...
                CP	TOF_exec		;CASE statement in WHEN line	
                JR	NZ,CASE1	
                LD	A,1	
                JR	CASE0	
;
...
WHICHK:         CALL	EXPRI	
                ...
                POP	BC		;Pop marker	
                POP	BC		;Level stack	
                LD	BC,TWHILE_exec+TENDWHILE_exec*256	
                LD	D,1	
                CALL	WSRCH	
XEQGO5:         JP	XEQ	
;
...
; EXIT WHILE
;
EXIT_exec:           INC	IY		;Skip FOR_exec/REPEAT/WHILE	
                CP	TFOR_exec	
                JR	NZ,EXIT0	
                LD	IX,0		;For EXIT_exec FOR_exec <var>	
                CALL	TERMQ	
                CALL	NZ,GETVAR	
                LD	A,TFOR_exec	
...
EXIT1:          LD	A,E	
                ...
                JR	NZ,EXIT1	
                LD	A,44	
                JP	ERROR_		;'Bad EXIT_exec'	
;
EXIT4:          POP	BC		;VARPTR	
                LD	HL,14		;Skip text pointer, limit & step	
                ADD	HL,SP	
                LD	SP,HL		;Pop FOR_exec record	
                CP	TFOR_exec	
                JR	NZ,EXIT1	
                PUSH	IX	
                POP	HL	
                ...
                JR	Z,EXIT5	
                SBC	HL,BC	
EXIT5:          LD	BC,TFOR_exec+TNEXT_exec*256	
                JR	Z,EXIT8	
                INC	D		;Count nested FOR_exec loops	
                JR	EXIT1	
;
EXIT6:          POP	BC		;Text pointer	
                CP	TREPEAT_exec	
                JR	NZ,EXIT1	
                LD	BC,TREPEAT_exec+TUNTIL_exec*256	
                JR	EXIT8	
;
...
XTRAC1:         LD	A,(IY)	
                ...
                JR	XTRAC1	
;
DSRCH:          LD	A,TDATA_exec	
SEARCH_exec:         LD	B,0	
SRCH1_exec:          LD	C,(HL)	
                INC	C	
                DEC	C	
                JR	Z,SRCH2_exec		;FAIL	
                INC	HL	
                INC	HL	
                INC	HL	
                CP	(HL)	
...
TFN:            EQU	0A4H	
TTO_main:            EQU	0B8H	
TWHILE_main:         EQU	0C7H	
TCASE:          EQU	0C8H	
TWHEN_main:          EQU	0C9H	
TOF_main:            EQU	0CAH	
TENDCASE_main:       EQU	0CBH	
TOTHERWISE_main:     EQU	0CCH	
TENDIF_main:         EQU	0CDH	
TENDWHILE_main:      EQU	0CEH	
TDATA_main:          EQU	0DCH	
TDIM_main:           EQU	0DEH	
TFOR_main:           EQU	0E3H	
TGOSUB_main:         EQU	0E4H	
TGOTO_main:          EQU	0E5H	
TIF:            EQU	0E7H	
TLOCAL_main:         EQU	0EAH	
TNEXT_main:          EQU	0EDH	
TON_main:            EQU	0EEH	
TPROC_main:          EQU	0F2H	
TREM_main:           EQU	0F4H	
TREPEAT_main:        EQU	0F5H	
TRESTORE:       EQU	0F7H	
TTRACE:         EQU	0FCH	
TUNTIL_main:         EQU	0FDH	
TEXIT_main:          EQU	0x10	
;
TOKLO:          EQU	0x8F	
TOKHI:          EQU	0x93	
OFFSET_main:         EQU	0CFH-TOKLO	
;
START:          JP	COLD	
                JP	WARM	
                JP	ESCAPE	
...
ERRWDS:         DB	0x17	
                ...
                DB	'#'	
                DB	0	
                DB	0x19		;46 Not in a WHILE loop	
                DB	TWHILE_main	
                DB	0x1A	
                DB	0	
                DB	0x11		;47 Missing ENDCASE	
                DB	TENDCASE_main	
                DB	0	
                DB	TOF_main		;48 OF not last	
                DB	0x1B	
                DB	"last"	
                DB	0	
                DB	0x11		;49 Missing ENDIF	
                DB	TENDIF_main	
                DB	0	
                DW	0	
                DB	0	
                DB	TON_main		;53 ON ERROR not LOCAL	
                DB	' '	
                DB	TERROR_main	
                DB	0x1B	
                DB	TLOCAL_main	
                DB	0	
                DB	TDATA_main		;54 DATA not LOCAL	
                DB	0x1B	
                DB	TLOCAL_main	
                DB	0	
;
...
;NEW
;
BAD_main:            CALL	TELL		;"Bad program'	
                DB	0x13	
                DB	"program"	
                DB	CR	
                DB	LF_main	
...
ERROR0_main:         CP	(HL)	
                ...
                JR	NZ,ERROR0_main	
                DJNZ	ERROR0_main	
                JR	ERROR1_main		;MUST NOT PUSH HL HERE	
;
EXTERR:         POP	HL	
                LD	C,A	
ERROR1_main:         LD	(ERRTXT),HL	
                LD	HL,(ONERSP)	
                LD	A,H	
                OR	L	
                ...
                JR	Z,ERROR4_main	
                LD	SP,HL	
ERROR4_main:         LD	A,C		;ERROR NUMBER	
                CALL	SETLIN		;SP IS SET NOW	
                LD	(ERR),A	
                LD	(ERL),HL	
                OR	A	
                JR	Z,ERROR2_main	;'FATAL' ERROR	
                LD	HL,(ERRTRP)	
                LD	A,H	
                OR	L	
                PUSH	HL	
...
PUTVAR:         CALL	CREATE	
                ...
                CP	')'		;WHOLE ARRAY_main?	
                JR	Z,GETV1	
ARRAY_main:          LD	A,14		;'Bad use of array'	
ERROR3_main:         JP	ERROR__main	
;
...
;
PAIR_main:           CALL	LINNUM		;FIRST	
                LD	A,H	
                OR	L	
                JR	NZ,PAIR1_main	
                LD	L,10	
PAIR1_main:          CALL	TERMQ	
                INC	IY	
                PUSH	HL	
                LD	HL,10	
...
LEXAN2:         LD	A,E		;MAIN ENTRY	
                CP	252		;TEST LENGTH	
                LD	A,19	
                JP	NC,ERROR__main	;'String too long'	
                LD	A,(IY)	
                CP	CR	
                RET	Z		;END OF LINE	
                CALL	RANGE1	
...
LEXAN7:         CP	'*'	
                ...
                CP	TOKHI+1	
                JR	NC,LEXAN8	
                ADD	A,OFFSET_main	;LEFT VERSION	
LEXAN8:         CP	TREM_main	
                JR	Z,LEXAN9	
                CP	TDATA_main	
                JR	NZ,LEXANA	
...
EXIT_math:           POP	IY		;Restore IY	
                RET			;Return to caller	
;
;Error exit:
;
BAD_math:            LD	A,BADOP		;"Bad operation code"	
ERROR__math:         LD	SP,IY		;Restore SP from IY	
                OR	A		;Set NZ	
                SCF			;Set C	
                JR	EXIT_math	
;
...
FTABLE:         DW	ABS		;16 ABS	
                DW	ACS_math		;17 ACS_math	
                DW	ASN_math		;18 ASN_math	
                DW	ATN_math		;19 ATN_math	
                DW	COS_math		;20 COS_math	
                DW	DEG_math		;21 DEG_math	
                DW	EXP_math		;22 EXP_math	
                DW	INT__math		;23 INT__math	
                DW	LN_math		;24 LN_math	
                DW	LOG_math		;25 LOG_math	
                DW	CPL__math		;26 NOT	
                DW	RAD_math		;27 RAD_math	
                DW	SGN_math		;28 SGN_math	
                DW	SIN_math		;29 SIN_math	
                DW	SQR_math		;30 SQR_math	
                DW	TAN_math		;31 TAN_math	
;
                DW	ZERO_math		;32 ZERO_math	
                DW	FONE		;33 FONE	
                DW	TRUE_math		;34 TRUE_math	
                DW	PI_math		;35 PI_math	
;
                DW	VAL_math		;36 VAL_math	
                DW	STR_math		;37 STR_math$	
;
                DW	SFIX_math		;38 FIX	
                DW	SFLOAT_math		;39 FLOAT	
;
                DW	FTEST		;40 TEST_math	
                DW	FCOMP		;41 COMPARE	
;
                DW	ISHL		;42 <<	
                DW	ISHX		;43 <<<	
...
FADD:           DEC	B	
                INC	B	
                RET	Z		;ARG 2 ZERO_math	
                DEC	C	
                INC	C	
                JP	Z,SWAP_math		;ARG 1 ZERO_math	
                EXX	
                LD	BC,0		;INITIALISE	
                EXX	
                LD	A,H	
                ...
                CALL	C,SWAP_math		;MAKE DED'E'B LARGEST	
                LD	A,B	
                SET	7,H		;IMPLIED 1	
                CALL	NZ,FIX		;ALIGN	
                POP	AF	
...
;
IDIV:           CALL	FLOAT2	
FDIV:           DEC	B		;TEST FOR_math ZERO_math	
                INC	B	
                LD	A,DIVBY0	
                JP	Z,ERROR__math		;"Division by zero"	
                DEC	C		;TEST FOR_math ZERO_math	
                INC	C	
                RET	Z	
                LD	A,H	
...
;
FMUL:           DEC	B		;TEST FOR_math ZERO_math	
                INC	B	
                JP	Z,ZERO_math	
                DEC	C		;TEST FOR_math ZERO_math	
                INC	C	
...
;
ABS:            BIT	7,H	
                RET	Z		;POSITIVE/ZERO_math	
                DEC	C	
                INC	C	
                JP	Z,NEGATE_math	;INTEGER	
                RES	7,H	
                RET	
;
;NOT - Complement integer.
...
INT__math:           DEC	C	
                ...
                EX	AF,AF'	
                AND	B	
                CALL	M,ADD1_math		;NEGATIVE NON-INTEGER	
                LD	A,B	
                OR	A	
                CALL	M,NEGATE_math	
                XOR	A	
...
;Result is floating-point numeric.
;
SQR_math:            CALL	SFLOAT_math	
SQR0:           BIT	7,H	
                LD	A,NGROOT	
                JP	NZ,ERROR__math	;"-ve root"	
                DEC	C	
                INC	C	
                RET	Z		;ZERO	
                SET	7,H		;IMPLIED 1	
...
EXP0:           CALL	LN2		;LN(2)	
                ...
                JP	C,ZERO_math	
                LD	A,EXPRNG	
                JP	ERROR__math		;"Exp range"	
;
EXP1:           AND	0x80	
                OR	E	
                PUSH	AF		;INTEGER PART	
                RES	7,H	
                CALL	PUSH5		;PUSH X*LN_math(2)	
                CALL	POLY	
                DW	4072H		;a(7)	
                DW	942EH	
                DB	0x73	
...
;Result is floating-point numeric.
;
LN_math:             CALL	SFLOAT_math	
LN0:            LD	A,LOGRNG	
                BIT	7,H	
                JP	NZ,ERROR__math	;"Log range"	
                INC	C	
                DEC	C	
                JP	Z,ERROR__math	
                LD	DE,3504H	;SQR(2)	
...
;Result is floating-point numeric.
;
ATN_math:            CALL	SFLOAT_math	
ATN0:           PUSH	HL		;SAVE SIGN	
                RES	7,H	
                LD	DE,5413H	;TAN(PI_math/8)=SQR_math(2)-1	
                EXX	
                LD	DE,0CCD0H	
                EXX	
                LD	B,0x7E	
                ...
                LD	B,0	
                JR	C,ATN2	
                LD	DE,1A82H	;TAN(3*PI_math/8)=SQR_math(2)+1	
                EXX	
                LD	DE,799AH	
                EXX	
                LD	B,0x81	
...
STR27:          INC	B	
                CALL	TEST_math		;IS HLH'L' ZERO_math?	
                LD	C,32	
                LD	A,0	
                JR	NZ,STR22	
                POP	AF	
...
;           A7 = 0 (numeric marker)
;
CON_math:            CALL	ZERO_math		;INITIALISE TO ZERO_math	
                LD	C,0		;TRUNCATION COUNTER	
                CALL	NUMBER_math		;GET INTEGER PART	
                CP	'.'	
                ...
                OR	A	
                JR	NZ,CON0		;EXPONENT NON-ZERO_math	
                CP	B	
...
NUMB1:          INC	C		;TRUNCATION COUNTER	
                CALL	SWAP1_math		;RESTORE PREVIOUS VALUE	
                JR	NUMBER_math	
;
...
SCALE:          LD	A,150	
                CP	C	
                LD	A,ACLOST	
                JP	C,ERROR__math		;"Accuracy lost"	
                CALL	PIBY4	
                EXX	
...
MOD48:          SET	7,D		;IMPLIED 1	
                ...
                LD	IX,0	
                PUSH	IX		;PUT ZERO_math ON STACK	
                CP	B	
...
INCC:           INC	C	
                RET	NZ	
OFLOW:          LD	A,TOOBIG_math	
                JP	ERROR__math		;"Too big"	
;
;FTEST - Test for zero & sign
;    Output: A=0 if zero, A=&40 if +ve, A=&C0 if -ve
;
FTEST:          CALL	TEST_math	
                RET	Z	
                LD	A,H	
                AND	10000000B	
                OR	01000000B	
...
ABS2:           EX	AF,AF'	
                BIT	7,H	
                CALL	NZ,NEGATE_math	;MAKE ARGUMENTS +VE	
                CALL	SWAP_math	
                BIT	7,H	
                CALL	NZ,NEGATE_math	
                LD	B,H	
